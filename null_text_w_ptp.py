...
def text2image_instructpix2pix_blend(
    ...
    image_processor = VaeImageProcessor(
        vae_scale_factor=model.vae_scale_factor)  # 图像预处理： 输入的图像会通过 VaeImageProcessor 进行处理。该处理器调整图像的大小并将其准备好输入到模型中。
    # 3. Preprocess image
    image = image_processor.preprocess(image)

    # gt_image_pil_style=image_processor.preprocess(gt_image_pil_style)
    ...
    if uncond_embeddings is None:
        ...
        uncond_embeddings_ = model.text_encoder(uncond_input.input_ids.to(model.device))[0]  # 无条件嵌入
    else:
        uncond_embeddings_ = None
    ...
    latents = model.prepare_latents(
        ...
        generator,
        latents,
    )  # 准备潜变量，为扩散模型生成过程提供初始化条件,扩散模型的初始化采样参数
    ...
    # optimizer = torch.optim.AdamW(text_encoder.get_input_embeddings().parameters(), lr=0.002)

    for i, t in enumerate(tqdm(model.scheduler.timesteps[-start_time:])):

        if uncond_embeddings_ is None:
            context = torch.cat([text_embeddings, uncond_embeddings[i].expand(*text_embeddings.shape),
        ...
        latent_model_input = torch.cat([latents_mb] * 3)  # 初始潜变量复制三次 因为扩散模型需要分别处理 文本条件潜变量、图像条件潜变量 和 无条件潜变量，因此需要 3 组相同的初始潜变量输入。
        scaled_latent_model_input = model_mb.scheduler.scale_model_input(latent_model_input,t)  # 扩散模型中的标准步骤，作用是将潜变量调整到与当前时间步一致的噪声尺度
        try:
            scaled_latent_model_input = torch.cat([scaled_latent_model_input, image_latents],dim=1)  # 将图像潜变量（image_latents）作为附加信息引入模型，为生成过程提供与输入图像相关的条件。
        except:
            import pdb;
            pdb.set_trace()

        # predict the noise residual # 预测噪声残差
        # return_dict=False：表示输出为张量而非字典
        noise_pred = model_mb.unet(scaled_latent_model_input, t, encoder_hidden_states=context, return_dict=False)[0]

        noise_pred_text, noise_pred_image, noise_pred_uncond = noise_pred.chunk(3)  # 将噪声预测 noise_pred 按第一维分解为三个部分：noise_pred_text：文本条件的噪声残差 noise_pred_image：图像条件的噪声残差。noise_pred_uncond：无条件（unconditional）噪声残差。
        # noise_pred 是一个张量，包含对噪声残差的预测。
        # guidance_scale (w_t)：文本条件的引导强度，用于调整生成内容对文本的依赖程度。image_guidance_scale (w_i)：图像条件的引导强度，用于调整生成内容对图像的依赖程度。组合目标：平衡文本、图像和无条件分量在噪声预测中的权重。
        ...
        # compute the previous noisy sample x_t -> x_t-1
        latents_mb = model_mb.scheduler.step(noise_pred, t, latents_mb, return_dict=False)[0]  # 利用扩散调度器根据当前噪声预测结果更新潜变量。调度器将潜变量从当前时间步xt推到上一时间步xt-1
        latents_mb = controller.step_callback(latents_mb)  # 在扩散过程中，controller_bld 的主要功能是：1.记录注意力分布(用于生成掩码)。2.动态调整潜变量的更新行为。3.通过 step_callback 在每一步生成中进行干预。4.这种机制为用户提供了对扩散生成过程的更细粒度控制，使生成结果更加精确和可解释。
        latent_mb_to_insert = latents_mb.detach().clone()  # 克隆复制
        # MagicBrush predict the noise residual latent_mb_to_insert

        # inference
        latent_model_input = torch.cat([latents_ip2p] * 3)
        scaled_latent_model_input = model.scheduler.scale_model_input(latent_model_input,t)  # 对潜变量输入进行缩放，以适配当前扩散步长 t 的噪声比例。
        try:
            scaled_latent_model_input = torch.cat([scaled_latent_model_input, image_latents], dim=1)
            # print("你好")
        except:
        ...
        # predict the noise residual
        noise_pred = model.unet(scaled_latent_model_input, t, encoder_hidden_states=context, return_dict=False)[0]

        # loss_pred = F.mse_loss(noise_pred.float(), noise.float(), reduction="mean")
        # loss_pred.backward()
        # optimizer.step()
        # lr_scheduler.step()
        # optimizer.zero_grad()
        ...
        noise_pred_text, noise_pred_image, noise_pred_uncond = noise_pred.chunk(3)
        noise_pred = (
                    noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_image) + image_guidance_scale * (
                        noise_pred_image - noise_pred_uncond))

        # 处理掩码
        mask_resized = F.interpolate(mask, size=(64, 64), mode='nearest')
        mask_resized = mask_resized.to(device, dtype=torch.float16)
        # 应用局部扩散指导
        # 参考代码集成：仅对掩码区域应用强引导
        # 计算引导噪声
        guided_noise = (noise_pred_text - noise_pred_image) * guidance_scale + (
                noise_pred_image - noise_pred_uncond) * image_guidance_scale
        # 应用掩码
        guided_noise = guided_noise * mask_resized + noise_pred_uncond * (1 - mask_resized)
        # 更新噪声预测
        noise_pred = noise_pred_uncond + guided_noise
        ...
        if mode == "REMOVE":
            ...
            beta = 0.01
            latents_ip2p = (latents_ip2p + beta * latent_mb_to_insert) / (1 + beta)
            # latents_ip2p = (0.5*latents_ip2p + 0.5 * latent_mb_to_insert) / (1 + beta)
            # latents_ip2p = latents_ip2p
            # latents_ip2p = latent_mb_to_insert
        torch.cuda.empty_cache()
